...
public class NMItemListPriceCalculator extends ItemListPriceCalculator {
  ...
  protected double getPrice(Object pPriceSource) throws PricingException {
    ...
    if (localizedPrice != null) {
      ...
      double listPrice = getPricingTools().round(localizedPrice);
      if (isLoggingDebug()) {
      ...
      if (isLoggingDebug()) {
        logDebug("commerce item " + ((GSAItem) pPriceSource).getRepositoryId() + " do not have a localized price in country " + countryCode + ",so getting the price from property "
                + getPricePropertyName() + " price is " + ((GSAItem) pPriceSource).getPropertyValue(getPricePropertyName()));
      }
      return super.getPrice(pPriceSource);
    }
